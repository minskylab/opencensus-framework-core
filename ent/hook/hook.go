...
// The BedRecordFunc type is an adapter to allow the use of ordinary
// function as BedRecord mutator.
type BedRecordFunc func(context.Context, *ent.BedRecordMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BedRecordFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BedRecordMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BedRecordMutation", m)
	}
	return f(ctx, mv)
}
...
// Mutate calls f(ctx, m).
func (f OxygenRecordFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
